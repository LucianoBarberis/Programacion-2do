...
{
    // Banco que administra múltiples cuentas
    public class Banco
    {
        private readonly Dictionary<string, CuentaBancaria> cuentas = new();
    ...
    class Program
    {
        static void Main()
        {
            ...
            {
                Console.Clear();
                Console.WriteLine("=== Sistema Bancario - Menú ===");
        ...
        static void CrearCuenta(Banco banco, bool tipoAhorro)
        {
            ...
            if (!decimal.TryParse(Console.ReadLine(), out var saldoInicial))
                throw new DatosInvalidosException("Saldo inicial inválido.");

            CuentaBancaria cuenta = tipoAhorro
                ? new CuentaAhorros(num, titular, saldoInicial)
                : new CuentaCorriente(num, titular, saldoInicial);
